...
pub struct MsgUser {
    pub rep: F,
    pub num_since_last_interaction: F,
}
...
type U = User<F, MsgUser>;
type UV = UserVar<F, MsgUser>;
...
// element).
type CB = Callback<F, MsgUser, CBArg, CBArgVar>;

// The first type of interaction (doing a method update, which does nothing so far)
type Int1 = Interaction<F, MsgUser, (), (), (), (), CBArg, CBArgVar, 1>;
...
type PubScan = PubScanArgs<F, MsgUser, F, FpVar<F>, Cr, GRSchnorrCallbackStore<F>, NUMSCANS>;
...
fn int_meth<'a>(tu: &'a U, _pub_args: (), _priv_args: ()) -> U {
    ...
    // We could update num_since_last_interaction here! If you want :), there's nothing enforcing we need to keep it
    // the same.
    a
}

// Enforce that rep in [0, 1, 2] (user can pretty much pick num_since_last_interaction). Additionally, enforce
// that rep is identical to (before + 1). This is effectively a rate limit: The user can
// only do an interaction twice before being unable to produce a proof.
//
// Note that there are no enforcements on num_since_last_interaction, so it can be whatever the user wants.
fn int_meth_pred<'a>(
    tu_old: &'a UV,
    tu_new: &'a UV,
    _pub_args: (),
    ...
    let l0 = tu_new.data.rep.is_eq(&FpVar::Constant(F::from(0)))?;
    let l1 = tu_new.data.rep.is_eq(&FpVar::Constant(F::from(1)))?;
    let l2 = tu_new.data.rep.is_eq(&FpVar::Constant(F::from(2)))?;
    let o2 = tu_old.data.rep.clone() + FpVar::Constant(F::from(1));
    let b2 = tu_new.data.rep.is_eq(&o2)?;
    Ok((l0 | l1 | l2) & b2)
}

// We can also prove arbitrary predicates to the server. Here, we just prove that our num_since_last_interaction value
...
fn some_pred<'a, 'b>(
    ...
    tu.data
        .num_since_last_interaction
        .is_eq(&FpVar::Constant(F::from(3)))
}

// The callback method here allows the *server* to call a method on the user: Here the argument (F)
// being passed in can be selected by the server, and the user is forced to set their rep to
// `args`.
fn cb_meth<'a>(tu: &'a U, args: F) -> U {
    let mut out = tu.clone();
    out.data.num_since_last_interaction = tu.data.num_since_last_interaction + args;
    out
}
...
fn cb_pred<'a>(tu_old: &'a UV, args: FpVar<F>) -> ArkResult<UV> {
    ...
    tu_new.data.num_since_last_interaction = tu_new.data.num_since_last_interaction + args;
    Ok(tu_new)
}
...
fn main() {
    ...
    let ex: PubScanArgs<F, MsgUser, F, FpVar<F>, Cr, GRSchnorrCallbackStore<F>, NUMSCANS> =
        PubScanArgs {
    ...
    let mut u = User::create(
        MsgUser {
            rep: F::from(0), // Try changing this to 1 or 2 to see what happens!
            num_since_last_interaction: F::from(3), // Try changing this off of 3 to see what happens!
        },
        &mut rng,
    );
    println!("[USER] Created! User: {:o}", u);
    println!("[USER] Created! User: {:?}", u);

    // Join in as a user
    let _ = <GRSchnorrObjStore as JoinableBulletin<F, MsgUser>>::join_bul(
        &mut store.obj_bul,
        u.commit::<Poseidon<2>>(),
        (),
    );
    ...
    let out = <GRSchnorrObjStore as UserBul<F, MsgUser>>::verify_interact_and_append::<
        (),
        Groth16<E>,
        1,
    >(
    ...
    let res = store
        .approve_interaction_and_store::<MsgUser, Groth16<E>, (), GRSchnorrObjStore, Poseidon<2>, 1>(
